...
def matrix_divided(matrix, div=1):
    """Return a new matrix which is the result of the original matrix divided by div.
    ...
    if (not isinstance(matrix, list) or matrix == [] or
        ...
        not all((isinstance(elem, int) or isinstance(elem, float)) for elem in [num for row in matrix for num in row])
    ):
